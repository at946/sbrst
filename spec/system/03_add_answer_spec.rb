...
feature "03_回答を登録できること", type: :system, js: true do
  ...
  feature "【ブレストページ】で", type: :system, js: true do
    ...
    scenario "【answer】を入力した状態で【Enter】を選択した場合、【answer list】の１番上に入力した文字が追加され【answer】がクリアされること" do
      ...
      expect(find("#answer_list").all(".answer").count).to eq 1
      expect(find("#answer_list").all(".answer")[0]).to have_text @answers[0]

      fill_in :answer, with: @answers[1]
      find("#answer").native.send_keys :return

      expect(page).to have_selector ".answer"
      expect(find("#answer_list").all(".answer").count).to eq 2
      expect(find("#answer_list").all(".answer")[0]).to have_text @answers[1]
      expect(find("#answer_list").all(".answer")[1]).to have_text @answers[0]

      fill_in :answer, with: @answers[2]
      find("#answer").native.send_keys :return

      expect(page).to have_selector ".answer"
      expect(find("#answer_list").all(".answer").count).to eq 3
      expect(find("#answer_list").all(".answer")[0]).to have_text @answers[2]
      expect(find("#answer_list").all(".answer")[1]).to have_text @answers[1]
      expect(find("#answer_list").all(".answer")[2]).to have_text @answers[0]
    end
  end
end
